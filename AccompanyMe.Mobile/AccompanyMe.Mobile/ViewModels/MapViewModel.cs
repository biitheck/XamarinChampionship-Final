...
using AccompanyMe.Mobile.CustomPins;
...
namespace AccompanyMe.Mobile.ViewModels
{
    public class MapViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        public MapViewModel()
        {
            this._mapCenter = new Position(18.919973, -99.247789);
            //this._mapRegion = new MapSpan(this._mapCenter, this.MapCenter.Latitude, this.MapCenter.Longitude);
		...
		public async void CurrentLocationAsync()
		{
			try
			{
				var locator = CrossGeolocator.Current;
				var currentLocation = await locator.GetPositionAsync(10000);
				MapCenter = new Position(currentLocation.Latitude, currentLocation.Longitude);
				MapRegion = MapSpan.FromCenterAndRadius(MapCenter, Distance.FromKilometers(1)).WithZoom(8);
			}
			catch (Exception ex)
			{
        ...
        public Command<Position> MapLongPressCommand
        {
            get
            {
                return new Command<Position>(async position =>
                {
                    ...
                    if (action == "Evento")
                    {
						NavigationService.Instance.NavigateTo<NewEventViewModel>(position);
                        //var pin = new TKCustomMapPin
                        //{
                        //    Position = position,
